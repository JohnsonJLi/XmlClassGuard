...
import com.xml.guard.utils.getSuffixName
...
//                obfuscateClassPath = "$obfuscatePackage.${flavor.capitalize()}${nameHandling(rawName)}"
                obfuscateClassPath = "$obfuscatePackage.${skitConfusionDictionaryList.random()}${nameHandling(rawName)}"
                val arr = rawClassPath.split("$")
                classMapping[arr[0]] = obfuscateClassPath
...
//                obfuscateClassPath = "$obfuscatePackage.${flavor.capitalize()}${nameHandling(rawName)}"
    ...
    private fun getSkitConfusionDictionary(): String {
        classIndex++
        skitConfusionDictionaryList[classIndex.toInt()]
        return classIndex.toUpperLetterStr()
    }

    private val skitConfusionDictionaryList = mutableListOf(
        "Scriptwriting",

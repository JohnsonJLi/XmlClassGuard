 ...
 * Time: 19:06
 */
open class FlavorXmlClassGuardTask @Inject constructor(
    @Internal val guardExtension: GuardExtension
) : DefaultTask() {
    ...
    fun execute() {
        ...
        androidProjects.forEach {
            handleResDir(it)
            if (!guardExtension.flavor.isNullOrEmpty()) {
                handleResDir(it, guardExtension.flavor!!)
            }
        }
    ...
    private fun guardXml(project: Project, xmlFile: File, guardExtension: GuardExtension) {
